...
// --- choose_room / choose_target, raised by an effect -----------------------

/** A game paused on a `choose_room` prompt an onEnter effect raised. */
function raisedChooseRoom(): { state: GameState; seat: string; content: Content } {
  const withPrompt = dtile(
    'tile.p_prompt_room',
    { n: true },
    {
...
describe('choose_room, raised by an effect', () => {
  it('offers ANSWER for every candidate, matching legalAnswersFor', () => {
    const { state, seat, content } = raisedChooseRoom();
    expect(state.pending!.kind).toBe('choose_room');
    const candidates = legalAnswersFor(state.pending!) ?? [];
    expect(candidates.length).toBeGreaterThan(0);

    const legal = getLegalActions(state, seat, content);
    expect(legal).toHaveLength(candidates.length);
    for (const action of legal) {
      expect(action.t).toBe('ANSWER');
    }
  });

  it('accepts a real candidate and rejects one that is not offered', () => {
    const { state } = raisedChooseRoom();
    const candidates = legalAnswersFor(state.pending!) as string[];
    expect(validateAnswer(state.pending!, candidates[0])).toBe(true);
    expect(validateAnswer(state.pending!, 'ground:99,99')).toBe(false);
  });

  it('ANSWER and the default-timeout resolution agree, same as rotate_tile', () => {
    const { state, seat, content } = raisedChooseRoom();
    const candidates = legalAnswersFor(state.pending!) as string[];
    ...
    const viaAnswer = reduce(
      state,
      {
        ...
        seat,
        promptId: state.pending!.id,
        answer: state.pending!.defaultAnswer,
      },
      content,
    );
    const armed = reduce(state, { t: 'TICK', now: T0 }, content);
    const viaTimeout = reduce(
      armed.state,
      { t: 'TICK', now: T0 + state.timers.promptMs },
      content,
    );

    expect(viaAnswer.error).toBeUndefined();
    expect(candidates).toContain(state.pending!.defaultAnswer);
    expect(viaAnswer.state.pending).toBeNull();
    expect(viaTimeout.state.pending).toBeNull();
    expect(viaAnswer.state.players).toEqual(viaTimeout.state.players);
  });

  it('checkInvariants is clean on a freshly raised choose_room prompt', () => {
    const { state } = raisedChooseRoom();
...
describe('prompt kinds that do not exist yet', () => {
  ...
  // choose_target/choose_room are raised now (the effect interpreter),
  // covered under their own describe block below with real payloads.
  const unraised = PROMPT_KINDS.filter(
    (k) => k !== 'rotate_tile' && k !== 'choose_target' && k !== 'choose_room',
  );
