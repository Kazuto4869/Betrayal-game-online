...
describe('MOVE_THROUGH: onEnter effects', () => {
  it('resolves the tile onEnter effect and appends its events after discovered/moved', () => {
    ...
    const g = startedGame({ content: c });
    const seat = g.state.activeSeat!;
    const before = g.state.players[seat]!.traits.knowledge;

    const res = reduce(g.state, { t: 'MOVE_THROUGH', seat, dir: 'n' }, c);
    expect(res.error).toBeUndefined();
    expect(res.events.map((e) => e.t)).toEqual(['discovered', 'moved', 'trait_changed']);
    expect(res.state.players[seat]!.traits.knowledge).toBeGreaterThan(before);
    expect(checkInvariants(res.state)).toEqual([]);
  });

  it('does nothing extra when the tile has no onEnter', () => {
    const c = discoveryContent(START_1DOOR, AUTO4);
    const g = startedGame({ content: c });
    const seat = g.state.activeSeat!;
