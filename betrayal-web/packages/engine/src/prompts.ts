...
/** Shared by `choose_target` and `choose_room`: both raise an
 * `EffectPromptPayload` whose `candidates` array is the whole answer space. */
function effectPromptHandler(kind: 'choose_target' | 'choose_room'): PromptHandler {
  return {
    validate(payload, answer) {
      if (!isEffectPromptPayload(payload) || payload.kind !== kind) return false;
      return kind === 'choose_room'
        ? (
            payload as Extract<EffectPromptPayload, { kind: 'choose_room' }>
          ).candidates.includes(answer as string)
        : (
            payload as Extract<EffectPromptPayload, { kind: 'choose_target' }>
          ).candidates.some((c) => targetRefEquals(c, answer));
    },
    legalAnswers(payload) {
      if (!isEffectPromptPayload(payload) || payload.kind !== kind) return [];
...
export const PROMPT_HANDLERS: Record<PromptKind, PromptHandler> = {
  ...
  choose_target: effectPromptHandler('choose_target'),
