...
/**
 * The suspension point for an effect list that raised a `choose_target` or
 * `choose_room` prompt (docs/08-haunt-system.md#83, the engine's
 * `runEffects`). Kept flat and explicit per `docs/05-engine.md#56` ("do not
 * attempt generators or coroutines"): `if` never appears here because its
 * branch is spliced into the effect list before suspension is considered,
 * and `for_each` supports exactly one level of mid-loop suspension via
 * `forEach` — a prompt inside a `for_each` nested in another `for_each`
 * is out of the interpreter's scope and it throws rather than mishandle it.
 *
 * `remaining`/`forEach.body` are `unknown[]`, not `Effect[]`: shared cannot
 * import @bahoth/content's `Effect` type without inverting the package
 * dependency direction (docs/03-architecture.md#dependency-rules). The
...
export interface EffectResume {
  ...
  forEach?:
    | {
        ...
        /** The `for_each`'s `do` template, reused for each seat. */
        body: unknown[];
        /** The seat whose `body` suspended partway through. */
        currentSeat: SeatId;
        /** What's left of `currentSeat`'s `body` after the prompt resolves. */
...
/**
 * `PendingPrompt.payload` when `kind` is `choose_target` or `choose_room`.
 * `candidates` is resolved by the interpreter from the `PromptSpec` selector
 * in content (docs/08-haunt-system.md#83's file comment) against live state
 * at the moment the prompt is raised — content authors a selector, not a
 * literal id list, since it cannot know which `PlacedId`s an unpredictable
 * board will have.
 */
...
export function isEffectPromptPayload(p: unknown): p is EffectPromptPayload {
  ...
  return typeof resume.actor === 'string' && Array.isArray(resume.remaining);
}
...
export interface GameState {
  ...
  /**
   * Game-scoped flags, for the `flag` condition and `set_flag` effect
   * (docs/08-haunt-system.md#83) at `scope: 'game'`. `PlayerState.flags` and
   * `PlacedTile.flags` already cover `'seat'`/`'tile'`; this is the one that
   * was missing.
   */
  flags: Flags;
  board: BoardState;
