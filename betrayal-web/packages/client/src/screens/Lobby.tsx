...
import { type GameAction, MIN_PLAYERS } from '@bahoth/shared';
        ...
        <button
          ...
          title={
            canStart
              ? undefined
              : seatId === hostSeat
                ? `Every player needs an explorer, and ${MIN_PLAYERS} player${MIN_PLAYERS > 1 ? 's' : ''} minimum`
              ...
              >
                ...
                <span className="charcard__traits">
                  {ALL_TRAITS.map((t) => (
                    <span
                      key={t}
                      className={`trait-pill trait-pill--${t}`}
                      title={`${TRAIT_LABELS[t]}: ${c.tracks[t][c.start[t]]}`}
                    >
                      <span className="trait-pill__icon" aria-hidden>
                        {TRAIT_ICONS[t]}
                      </span>
                      <span className="trait-pill__name">{TRAIT_SHORT[t]}</span>
                      <span className="trait-pill__val">{c.tracks[t][c.start[t]]}</span>
                    </span>
                  ))}
                </span>
