        ...
        <section className="panel game__players">
          ...
          <ul className="seatlist">
            {state.turnOrder.map((sid) => {
              ...
              if (!p) return null;
              const isActive = sid === state.activeSeat;
              const isMe = sid === seatId;
              return (
                <li
                  key={sid}
                  className={`seat seat--game${isActive ? ' seat--active' : ''}${isMe ? ' seat--me' : ''}`}
                >
                  ...
                  {character && (
                    <div className="seat__traits" aria-label="Traits">
                      {ALL_TRAITS.map((t) => {
                        const val = traitValue(state, sid, t, content);
                        return (
                          <span
                            key={t}
                            className={`trait-chip trait-chip--${t}`}
                            title={`${TRAIT_LABELS[t]}: ${val}`}
                          >
                            <span className="trait-chip__icon" aria-hidden>
                              {TRAIT_ICONS[t]}
                            </span>
                            <span className="trait-chip__label">{TRAIT_SHORT[t]}</span>
                            <strong className="trait-chip__value">{val}</strong>
                          </span>
                        );
                      })}
                    </div>
                  ...
                  {isActive && !p.isDead && (
                    <div className="seat__moves">
                      <span className="moves-chip" title="Remaining moves this turn">
                        <span aria-hidden>👟</span> Moves left:{' '}
                        <strong>{p.movesLeft}</strong>
                      </span>
                    </div>
                  )}
