...
**Last updated: 2026-08-09** · **Current milestone: M3 (in progress)**
...
| M3 — Cards, traits, dice         | 🟨 In progress | Effect interpreter landed first, per the roadmap's own ordering  |
...
Started with the `Effect` interpreter rather than content, per
`docs/09-roadmap.md`'s own risk table: "build it once in M2 for tile
rotation, generalise it in M3 inside the effect interpreter, before any
haunt depends on it." Item one is done; decks, dice, items, and the haunt
roll are still ahead.

### The effect interpreter

`EffectSchema` was `z.unknown()` since M2 — deliberately, because validating
against a union that didn't have an interpreter yet would wave through
exactly the typos it exists to catch. It is now a real discriminated union
(`@bahoth/content`'s `effects.ts`) walked by a real interpreter
(`engine/src/effects.ts`), scoped to what current state can actually
execute: no card decks, dice, or monster defs exist yet, so `roll`,
`give_card`/`take_card`, `spawn_monster`/`kill`, and the `script` escape
hatch are absent from the schema entirely rather than accepted and silently
unexecutable. `docs/08-haunt-system.md#83` records the full scope cut and
two deliberate deviations from that doc's sketch (`Branch` was never defined
anywhere, so `prompt` uses `then: Effect[]` instead; `for_each` needed a
`{ ref: 'each' }` `SeatRef` case the doc didn't provide).

Wired into exactly one call site this pass: `finishDiscovery`'s step 6,
resolving a newly-discovered tile's `onEnter`. `MOVE` (walking rooms already
on the board) is not wired — see the "known limitation" already on record
below D7 about which of several equal-length paths gets walked; giving rooms
`onEnter` effects is what makes that player-visible, and fixing it needs an
explicit `via` path from the client first.

Four decisions worth recording:

- **Suspension is flat, per `docs/05-engine.md#56`'s own instruction** ("do
  not attempt generators or coroutines"). `if` never appears in resume
  state — its branch is spliced into the effect queue being processed
  before suspension is considered, so a prompt inside an `if` is
  indistinguishable from one at the top level. `for_each` supports exactly
  one level of mid-loop suspension (it remembers which seats are left and
  what's left of the current seat's body); a prompt inside a `for_each`
  nested in another `for_each` throws rather than silently losing the outer
  loop's state.
- **`choose_target`/`choose_room` resolve from a content-authored
  _selector_, not a literal candidate list.** A tile's `onEnter` cannot know
  which `PlacedId`s an unpredictable board will have, so `PromptSpec` in
  content is `{ kind, among }` (e.g. `'reachable_from_actor'`,
  `'other_seats_in_room'`), and the interpreter resolves it against live
  state — reusing `getReachable` from `movement.ts` rather than
  reimplementing reachability — at the moment it raises the prompt. The
  resolved candidates live in the runtime `PendingPrompt` payload, not in
  content.
- **A prompt with one candidate auto-applies**, same reasoning as
  `rotate_tile`'s dedupe-by-effective-doors from M2: a choice with no
  consequence is worse than no choice. A prompt with zero candidates skips
  with a `log` event instead of raising anything.
- **`GameState` gained a top-level `flags: Flags`.** `PlayerState.flags` and
  `PlacedTile.flags` already existed for `'seat'`/`'tile'` scope; `'game'`
  scope had nowhere to write until now.

**One bug the test suite caught on the first real exercise of the code it
was written to exercise** — the kind of thing `docs/10-testing-and-ops.md`
calls the whole point of a test. The `for_each` mid-loop suspend/resume path
had two independent construction sites (`runForEach`'s own suspension, and
`resumeEffects`'s loop resuming the remaining seats) that were meant to
produce the same resume shape, but only one of them actually filled in
`currentSeat`/`currentRemaining` — the other left them undefined, which
`resumeEffects` would only discover the next time it tried to resume,
`unknown[]` where it expected an array. The very first test exercising a
three-seat `for_each` suspending mid-loop hit it immediately. Both
construction sites now go through one `suspendForEach` helper, so there is
exactly one place that shape gets built.

282 tests, up from 258: 20 new for the interpreter in isolation
(`effects.test.ts`), 3 for the `Floor`/`Trait` duplication tripwire
(`content.test.ts`), and extensions to `reduce.test.ts` (onEnter wiring,
watched failing by reverting the call) and `prompts.test.ts`
(`choose_room`'s legality, validation, and ANSWER/timeout parity, mirroring
the `rotate_tile`/`ROTATE_TILE` "two doors into one function" test from
PR #19).

**Not done, deliberately.** The `count` condition and its `CountableRef` —
referenced in `docs/08-haunt-system.md` but never defined there or anywhere
else in `docs/`; inventing it now, with no card or haunt asking for it,
would be guessing. `MOVE`'s `onEnter` wiring — tracked above. Drop-items-on-
death, part of the separate "trait tracks: death detection" roadmap item —
the `trait` effect detects death (it has to, to keep invariant 5 satisfied)
but does not yet drop the dead explorer's items.
...
1. ~~**Start M3 with the `Effect` interpreter**, not with content.~~ Done —
   see "M3 — in progress" above. `choose_target`/`choose_room` payloads
   turned out to be public info (same reasoning as `rotate_tile`'s), so
   nothing needed deciding there; `choose_card` still does, per item 3
   below.
2. **Answer roadmap open question 3** — whether any redistributable room set
   exists — before hand-entering 44 tiles and ~65 cards from a physical copy.
   Ten minutes of looking against hours of transcription, and content is the
   next M3 item that needs it (decks, then items/traits/dice).
3. **Decide what a prompt payload reveals, once `choose_card` exists.**
   `redactFor` passes `pending` through whole. Correct for `rotate_tile` and
   now `choose_target`/`choose_room`, whose payloads are drawn on everyone's
   board anyway; wrong the moment `choose_card` puts card identities in a
   payload. The rule belongs with the first kind that needs it, not before.
...
| Tests               | 49                    | 88                    | 258                   | 282                   |
| Packages            | 5                     | 5                     | 5                     | 5                     |
| Haunts implemented  | 0 / 50                | 0 / 50                | 0 / 50                | 0 / 50                |
| Room tiles authored | 0 / 44                | 0 / 44                | 44 / 44 (placeholder) | 44 / 44 (placeholder) |
| Cards authored      | 0 / ~65               | 0 / ~65               | 0 / ~65               | 0 / ~65               |
| Characters authored | 12 / 12 (placeholder) | 12 / 12 (placeholder) | 12 / 12 (placeholder) | 12 / 12 (placeholder) |
