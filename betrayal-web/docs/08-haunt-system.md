...
**As built for M3's first item** (`@bahoth/content`'s `effects.ts`,
`engine/src/effects.ts` — flat, not under a `systems/` directory the rest of
the engine doesn't actually use):

- Only `trait`, `move`, `set_flag`, `place_token`, `prompt`, `end_turn`,
  `log`, `if`, and `for_each` are real. `roll`, `give_card`, `take_card`,
  `spawn_monster`, `kill`, and `script` are absent from `EffectSchema`
  entirely, not accepted-but-inert — each needs a subsystem (dice, decks,
  monsters, the M4/M6 script registry) that doesn't exist yet, and
  authoring one today is a content-validation error. Same for the `count`
  condition: its `CountableRef` is referenced above but never defined
  anywhere in `docs/`.
- **`Branch` is never defined anywhere in `docs/` either**, so `prompt`'s
  `then` is `Effect[]`, run unconditionally after any valid answer, with the
  answer available to `then`'s effects via a new `{ ref: 'chosen' }` case on
  `SeatRef`/`RoomRef`. `roll` (deferred) would need to resurrect `Branch`
  for real when it lands.
- **`for_each` needed a way to target the seat one iteration is on**, which
  neither `who` nor anything in `SeatRef` provides — `who` only picks the
  set. `{ ref: 'each' }` fills that gap.
- **Suspension is flat**, per this section's own resume-cursor note: `if`
  never appears in resume state (its branch is spliced into the effect
  queue before suspension is considered), and `for_each` supports exactly
  one level of mid-loop suspension. A `prompt` inside a `for_each` nested in
  another `for_each` throws rather than silently losing the outer loop.
- **`GameState` gained a top-level `flags: Flags`** for `scope: 'game'` —
  `PlayerState.flags`/`PlacedTile.flags` already covered `'seat'`/`'tile'`.
- Wired into exactly one call site: `finishDiscovery`'s step 6 (a newly
  discovered room's `onEnter`). `MOVE` (walking already-discovered rooms) is
  deliberately not wired — see `docs/11-progress.md`'s note on why `MOVE`
  needs an explicit `via` path first.
